...
test.describe("User Signup", () => {
  test("should allow new user signup with email and password", async ({
    ...
    // Check if we're still on the signup page (error occurred) or redirected (success)
    ...
    if (currentUrl.includes("/sign-up")) {
      ...
      const errorElements = await page
        ...
        .count();
      if (errorElements > 0) {
        // There's an error message, test fails
        ...
        // No error message but still on signup page
        throw new Error(
          "Signup submission did not redirect and no error message found",
        );
      }
    } else {
