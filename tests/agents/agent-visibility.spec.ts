...
test.describe("Agent Visibility and Sharing Between Users", () => {
  test("first user creates agents with different visibility levels", async ({
    browser,
  }) => {
    // Create first user context (using admin auth for test setup)
    ...
    try {
      // Create public agent
      await firstUserPage.goto("/agent/new");
      await firstUserPage.waitForLoadState("networkidle");

      await firstUserPage.getByTestId("agent-name-input").fill(publicAgentName);
      await firstUserPage
      ...
      await firstUserPage
        ...
        .first()
        .click();
      await firstUserPage.waitForURL("**/agent/**", { timeout: 10000 });

      // Open visibility dropdown and select public
      await openDropdown(firstUserPage, "visibility-button");
      await selectDropdownOption(firstUserPage, "visibility-public");
      ...
      await openDropdown(firstUserPage, "visibility-button");
      await selectDropdownOption(firstUserPage, "visibility-readonly");
