...
test.describe("Admin Users List", () => {
  ...
  test("should search for users", async ({ page }) => {
    ...
    await expect(rows.first()).toContainText("Test Admin");
  });
  ...
  test("should show banned user status", async ({ page }) => {
    // Search for the banned test user (User 15)
    const searchInput = page.getByTestId("users-search-input");
    ...
    // Check banned status is displayed
    const bannedBadge = page.getByTestId("status-badge-banned");

    // If banned user exists, verify the badge is visible
...
test.describe("Admin Users List - Permissions", () => {
  test("non-admin user cannot access admin panel", async ({ browser }) => {
    ...
    // Could be redirected to home or show an error
    const url = page.url();
  ...
  test("admin can see admin panel", async ({ page }) => {
    ...
    // Should have clickable user rows
    const rows = page.locator("[data-testid='users-table'] tbody tr");
    const rowCount = await rows.count();
    expect(rowCount).toBeGreaterThan(0);
  });
