...
// Helper to create an agent with proper data-testid selectors
...
test.describe("Agent Creation and Sharing Workflow", () => {
  test("should create a new agent successfully", async ({ page }) => {
    ...
    // Verify we're on the new agent page
    expect(page.url()).toContain("/agent/new");

    // Wait for the form to be ready (not loading/saving)
    await page.waitForSelector(
      '[data-testid="agent-save-button"]:not([disabled])',
      { timeout: 10000 },
    );

    // Fill in agent details using data-testid
    const nameInput = page.locator('[data-testid="agent-name-input"]');
    await nameInput.waitFor({ state: "visible" });
    ...
    // Optional: fill in role (the "expert in" field)
    ...
    // Save the agent - wait for button to be enabled
    const saveButton = page.locator('[data-testid="agent-save-button"]');
  ...
  test("should navigate to agent from agents list", async ({ page }) => {
    const agentName = `Clickable Agent ${Date.now()}`;
  ...
  test("should edit an existing agent", async ({ page }) => {
    ...
    const originalName = `Original Agent ${Date.now()}`;
