...
export function AppSidebarAgents() {
  ...
  const visibleMyAgents = expanded ? myAgents : myAgents.slice(0, 3);
  const visibleBookmarkedAgents = expanded
    ? bookmarkedAgents
    : bookmarkedAgents.slice(0, 2);
  ...
  const handleAgentClick = useCallback(
    (id: string) => {
      ...
      if (currentThreadId) {
        // Case 2: Add agent to existing chat
        appStore.setState((prev) => {
          ...
          if (target) {
            return prev; // Agent already added
          }
        ...
        // Case 1: Start new chat with agent selected
        // Navigate first, then set the agent in a callback
        router.push("/");

        // Set the agent mention immediately for the new thread that will be created
        const newMention: ChatMention = {
          type: "agent",
          agentId: agent.id,
          name: agent.name,
        ...
        // We'll set this for the next thread that gets created
        appStore.setState((_prev) => ({
          pendingAgentMention: newMention,
        }));
      }
    },
    [agents, router],
          ...
          {isLoading && agents.length === 0 ? (
            ...
            <>
              <div className="flex flex-col gap-1 max-h-[30vh] overflow-y-auto">
                ...
                {visibleBookmarkedAgents.length > 0 && (
                  <>
                    <Separator className="my-1" />
                    {visibleBookmarkedAgents.map((agent, i) => {
                      const isLast =
                        i === visibleBookmarkedAgents.length - 1 && expanded;
                      return (
                        <SidebarMenu
                          key={agent.id}
