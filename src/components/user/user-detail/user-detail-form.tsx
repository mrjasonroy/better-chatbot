...
export function UserDetailForm({
  ...
  >(async (prevState, formData) => {
    ...
    if (result?.success && result.user) {
      ...
      // If user is updating their own profile, refresh session to update sidebar
      if (user.id === currentUserId) {
        try {
          // Refresh the session cache and trigger a router refresh
    ...
    <>
      ...
      <div className="flex-shrink-0 bg-background border-t border-border p-4">
        <div className="max-w-2xl mx-auto flex items-center justify-end gap-2">
          <UpdateUserPasswordDialog userId={user.id} view={view}>
            <Button
              variant="secondary"
