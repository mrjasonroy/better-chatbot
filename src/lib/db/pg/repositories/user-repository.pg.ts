...
export const pgUserRepository: UserRepository = {
  ...
  getUserStats: async (userId: string) => {
    ...
    const modelStats = await db
      .select({
        model: sql<string>`COALESCE(${ChatMessageSchema.model}, 'unknown')`,
        messageCount: count(ChatMessageSchema.id),
        // Extract usageTokens from the first annotation object in the array
        totalTokens: sql<number>`COALESCE(SUM((${ChatMessageSchema.annotations}[1]::json->>'usageTokens')::numeric), 0)`,
      })
      ...
      .where(
        sql`${ChatThreadSchema.userId} = ${userId} AND ${ChatMessageSchema.createdAt} >= ${thirtyDaysAgo}`,
      )
      .groupBy(ChatMessageSchema.model)
      .orderBy(
        sql`SUM((${ChatMessageSchema.annotations}[1]::json->>'usageTokens')::numeric) DESC`,
      )
