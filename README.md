...
# === CN-Specific UI Features (build-time) ===
# These NEXT_PUBLIC_* vars are baked into the client JS bundle at build time.
# Set in .env.{dev,production,staging} for Docker builds, or .env.local for local dev.
NEXT_PUBLIC_CN_AGENT_REQUIRED=    # Require agent selection before sending messages
NEXT_PUBLIC_CN_SHARE_LINK=        # Show share icon in chat header + prompt bar
NEXT_PUBLIC_CN_WIDE_TABLES=       # Tables expand to full available width
NEXT_PUBLIC_CN_DISCREET_MCP=      # MCP tool calls collapse to single discreet line
```
...
|------|-----------|---------|
| `.env` | No | Generated by `pnpm i`, local secrets (API keys, DB creds) |
| `.env.local` | No | Local overrides with secrets — **never commit** |
| `.env.dev` | Yes | Public build-time config for dev Docker builds (`NEXT_PUBLIC_*` only) |
| `.env.production` | Yes | Public build-time config for production Docker builds |
| `.env.staging` | Yes | Public build-time config for staging Docker builds |
| `.env.example` | Yes | Template showing all available variables |

> **Important:** `.env.dev`, `.env.production`, and `.env.staging` must only contain `NEXT_PUBLIC_*` variables — never secrets. Secrets are injected at runtime via Vault/ConfigMap.
